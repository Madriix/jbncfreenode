                ...
                for (let n = 0; n < lines.length; n++) {
                    ...
                    switch (s) {
                        case '001': // RPL_WELCOME
                            ...
                            this.client.realserver = data[0].slice(1);
                    ...
                    if (global.MSG_REDISTRIBUTION && this.client.messagetags && lines[n].startsWith("@")) {
                        ...
                        */
                        ...
                        // Here it only intercepts PRIVMSG or NOTICE
                        if (lines[n].split(" ")[2] === "PRIVMSG" || lines[n].split(" ")[2] === "NOTICE") {
                                ...
                                if (Object.prototype.hasOwnProperty.call(this.client.buffers, key)) {
                                    ...
                                    for (let x = 0; x < this.client.buffers[key].privmsgnotice.length; x++) {
                                        if (this.client.buffers[key].privmsgnotice[x].target === _n) {
                                            ...
                                            if (count > global.MSG_REDISTRIBUTION_LINEMAX) {
                        ...
                        } // Here it intercepts what is in global.ircCommandRedistributeMessagesOnConnect (AWAY, NICK, TOPIC...)
                            ...
                            for (let key in this.client.buffers) {
                                if (Object.prototype.hasOwnProperty.call(this.client.buffers, key)) {
                                    let _n = "server.irc";
                                    if (lines[n].split(" ")[2].trimEnd() === "MODE" && lines[n].split(" ")[3].trimEnd() === this.client.nick) {
                                        continue;
                                    }
                                    let count = 0;
                    ...
                    } // It intercepts server notices - Check the last 300 messages that will be sent on the irc web client to sort the new messages - 
                    else if (global.MSG_REDISTRIBUTION && this.client.messagetags && new RegExp(`^:${this.client.realserver} NOTICE`).test(lines[n]) && !this.client.connected) {

                        if (lines[n].split(" ")[1] === "NOTICE") {
                            for (let key in this.client.buffers) {
                                if (Object.prototype.hasOwnProperty.call(this.client.buffers, key)) {
                                    let _n = lines[n].split(" ")[2]; // PRIVMSG <hereTarget> :... - Channel name or PV name
                                    let source = lines[n].split(" ")[2].slice(1);
                                    let count = 0;
