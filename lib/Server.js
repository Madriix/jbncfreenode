...
class Server {
    ...
    handleData(socket, chunk) {
        const firstLine = chunk.toString().split('\n')[0]; // Première ligne des données
        // Vérifie si ça ressemble à une requête HTTP (ex: "GET /", "POST /", etc.)
        if (firstLine.match(/^(GET|POST|PUT|DELETE|HEAD|OPTIONS) \//)) {
            socket.write(
                'HTTP/1.1 400 Bad Request\r\n' +
                'Content-Type: text/plain\r\n' +
                'Connection: close\r\n' +
                '\r\n' +
                'Erreur : Ce serveur n\'accepte pas les connexions HTTP.\r\n'
            );
            socket.end();
            return;
        }
                                    ...
                                    if (/(.*)\|\|(.*)\/(.*)\|\|(.*)\/(.*)\/(.*)/g.test(passIrc)) {
                                        ...
                                        if (global.BOUNCER_MODE == "gateway") {
                                            ...
                                            if (origin.length != 2 && origin.length != 3 && origin.length != 4)
                                                ...
                                                let _server_pass = origin[1].split("||");
                                                let _server = _server_pass[0].split(":");
                            ...
                            if (!command[1]) {
                                ...
                                switch (command[1].toUpperCase().trim()) {
                                    ...
                                    case 'BUFFERS':
                                        ...
                                        for (let key in this.connections[socket.hash].buffers) {
                                            if (this.connections[socket.hash].buffers.hasOwnProperty(key)) {
                                                let connected = this.connections[socket.hash].buffers[key].connected;
                                    ...
                                    case 'PASS':
                                        if (command[3]) {
                                            if (hash(command[2]) == this.connections[socket.hash].password) {
                                                ...
                                                let _newhash = hash(socket.irc.nick + socket.irc.password + socket.irc.server + socket.irc.port.toString());
